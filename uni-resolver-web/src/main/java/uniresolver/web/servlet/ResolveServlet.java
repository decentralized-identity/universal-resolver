...
public class ResolveServlet extends WebUniResolver {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ServletUtil.sendResponse(response, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, null, "Resolve problem for " + didString + ": " + ex.getMessage());
			return;
		}
		...
		if (resolveResult == null || resolveResult.getDidDocumentStream() == null) {
			...
			ServletUtil.sendResponse(response, HttpServletResponse.SC_NOT_FOUND, null, "No resolve result for " + didString + ": " + resolveResult);
			return;
		}
		...
		for (MediaType acceptMediaType : acceptMediaTypes) {
			...
			if (acceptMediaType.includes(MediaType.valueOf(ResolveResult.MEDIA_TYPE))) {
				...
				if (resolveResult.getDidDocumentStream() == null) {

					ServletUtil.sendResponse(response, HttpServletResponse.SC_NOT_FOUND, null, "No content for " + didString);
					return;
				}
